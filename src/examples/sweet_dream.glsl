...
float sweetDreamSynth (float t) {
  ...
  float sound = 0.0;
  for (int i=0; i<16; ++i) {
    sound += synth(t, noteToFreq(float(notes[i]))) * adsr(m, vec4(0.05, 0.1, 0.7, 0.2), float(i), 0.5);
  }
  return sound;
}

float dsp(float t) {
  return 0.5 * beat(mod(t * bps, 1.0), 0.2, 60.0) +
         0.1 * adsr(mod(t * bps, 1.0), vec4(0.02, 0.05, 0.7, 0.2), 0.5, 0.1) * rand(t) +
